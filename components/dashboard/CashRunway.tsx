...
function ForecastMatrix({
  ...
  function LineRows({ category, groups }: { category: LineCategory; groups: string[] }) {
    ...
    const sectionLabel = isReceipt ? 'CASH RECEIPTS' : 'CASH DISBURSEMENTS';
    ...
    return (
      <>
        ...
        {groups.map(group => {
          const groupLines = lines.filter(l => l.category === category && l.group === group);
                  ...
                  return (
                    <td key={w.num} className="px-2 py-1.5 text-right">
                      <span className={`text-[11px] tabular-nums ${
                        amt === 0 ? 'text-slate-700' :
                        isReceipt  ? 'text-emerald-400/80' : 'text-red-400/80'
                      }`}>
      ...
      <div className="px-4 py-3 border-b border-slate-800/40 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-[10px] text-slate-600 mt-0.5">
            All figures derived from financial statements · Wk {minBalWeek} = minimum balance ({fmt(minBal, true)})
          </div>
        </div>
        {minBal < 0 && (
        ...
        <table className="w-full text-[11px]" style={{ minWidth: '1120px' }}>
          ...
          <tbody>
            ...
            <tr className="border-b border-slate-800/40 bg-slate-900/30">
              <td className="px-4 py-2 sticky left-0 bg-slate-900/50">
                ...
                <div className="text-[9px] text-slate-600 mt-0.5">↳ {
                  data_placeholder /* handled by parent */
                }</div>
              </td>
            ...
            <tr className="border-b border-slate-700/40 bg-slate-800/20">
              ...
              <td className={`px-3 py-2.5 text-right text-[11px] font-bold tabular-nums bg-slate-800/30 border-l border-slate-700/40 ${summary.reduce((s, ws) => s + ws.netCash, 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {fmt(summary.reduce((s, ws) => s + ws.netCash, 0), true)}
              </td>
            </tr>
...
// placeholder to be replaced — see main component
